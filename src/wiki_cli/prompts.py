...
- Otherwise, append one short paragraph, two to three sentences, at the end of the file, \
after a blank line separating it from existing content, without rewriting or reformatting \
the rest of the file. Open with a brief description: `.wiki/` is this repository's \
source-grounded knowledge base of its architecture, domains, and operations. Then instruct \
rather than merely describe: say to check `.wiki/index.md` first, **before** using \
Read/Grep/Glob to explore the codebase or answering *any* question about this repository — \
including simple ones, not only large changes — because the wiki already routes to the right \
page for most tasks. State that the code is authoritative over the wiki wherever they \
disagree.
